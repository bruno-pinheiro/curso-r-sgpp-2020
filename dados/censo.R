...
if (!file.exists("dados/censo-raw")){
  dir.create("dados/censo-raw")
  download.file("ftp://ftp.ibge.gov.br/Censos/Censo_Demografico_2010/Resultados_do_Universo/Agregados_por_Setores_Censitarios/SP_Capital_20190823.zip",
                destfile = "dados/censo-raw/SP_Capital_20190823.zip")
  unzip("dados/censo-raw/SP_Capital_20190823.zip", junkpaths = TRUE, exdir = "dados/censo-raw")
}

# DELETAR ARQUIVOS QUE NÃO SERÃO USADOS

lista_remocao <- list.files("dados/censo-raw")[
  !grepl("Pessoa03.*csv|Domicilio01.*csv|DomicilioRenda.*csv|SP_Capital",
         list.files("dados/censo-raw"))
  ]
